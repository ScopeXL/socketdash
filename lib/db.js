...
var db = new sqlite3.Database('./lib/db/db.sqlite3');
...
module.exports = function() {
    ...
    this.getServerEmitsChartData = function(currentTimestamp, clientId) {
        var p = new promise(function(resolve, reject) {
            ...
            db.serialize(function() {
                ...
                if (_.isUndefined(clientId) || _.isNull(clientId)) {
                    db.each('SELECT * FROM server_emits WHERE timestamp > ? ORDER BY timestamp DESC',
                        ...
                        function(err, row) {
                            ...
                            if (serverResults.length <= 0) {
                                serverResults.push([moment().unix() * 1000, 0]);
                            }
                            if (serverResults.length < 100) {
                                var ts = serverResults[0][0];
                                for (var i = 0; i < 100; i++) {
                                    ts = ts - 10000;
                                    serverResults.unshift([ts, 0]);
                                }
                            }
                    ...
                    db.each('SELECT * FROM server_emits WHERE timestamp > ? AND client_id = ? ORDER BY timestamp DESC',
                        [maxData, clientId],
                        function(err, row) {
                            // first row, set the interval to 10 seconds
                            if (isFirst) {
    ...
    // get chart data for server emits
    this.getClientEmitsChartData = function(currentTimestamp, clientId) {
        var p = new promise(function(resolve, reject) {
            var clientResults = [];
            ...
            db.serialize(function() {
                ...
                if (_.isUndefined(clientId) || _.isNull(clientId)) {
                    db.each('SELECT * FROM client_emits WHERE timestamp > ? ORDER BY timestamp DESC',
                        ...
                        function(err, row) {
                            ...
                            if (clientResults.length <= 0) {
                                clientResults.push([moment().unix() * 1000, 0]);
                            }
                            if (clientResults.length < 100) {
                                var ts = clientResults[0][0];
                                for (var i = 0; i < 100; i++) {
                                    ts = ts - 10000;
                                    clientResults.unshift([ts, 0]);
                                }
                            }
                    ...
                    db.each('SELECT * FROM client_emits WHERE timestamp > ? AND client_id = ? ORDER BY timestamp DESC',
                        [maxData, clientId],
                        function(err, row) {
                            // first row, set the interval to 10 seconds
                            if (isFirst) {
